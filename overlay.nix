...
let
  ...
  manifoldpkgs = { dapptoolsOverrides ? {} }: rec {
    ...
    manifoldCommonScriptBins = with self; [
    ...
    manifoldScriptPackage = self.callPackage ./script-builder.nix {};
  };
in (manifoldpkgs { inherit dapptoolsOverrides; }) // {
  manifoldpkgs = makeOverridable manifoldpkgs { inherit dapptoolsOverrides; };
}
